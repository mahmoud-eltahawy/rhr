...
function replaceButtons(catName: string ,fieldId: string){
  let arr : number[] | undefined = jsonMap.get(catName)
  if(arr){
    if(arr.length === 1){
...
function getCategoriesContainers() : Map<string,{vsize: number,catnum: {cat: string, num: number}}>{
  const mList = new Map()
...
function getCategoriesNumbersContainers() : Map<string,{cat: string, num: number}> {
